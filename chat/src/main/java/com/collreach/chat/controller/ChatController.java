...
import com.collreach.chat.model.response.ContactsResponse;
import com.collreach.chat.model.response.MessageResponse;
...
//    }
    ...
    public List<ContactsResponse> getAllContactsOfUser(@RequestParam(value = "username") String username,
        ...
        if(room.size() > 0){
            for(Room each: room){
                ...
                if(memberOne.getUsername().equalsIgnoreCase(username)){
                    contacts.add(new ContactsResponse(memberTwo, each.getLastMessage(), each.getLastContact()));
                }
                else{
                    contacts.add(new ContactsResponse(memberOne, each.getLastMessage(), each.getLastContact()));
                }
            }
        }
        return contacts;
    ...
    public List<MessageResponse> getAllSortedMessage(@RequestParam(value = "memberOne") String memberOne,
                                             @RequestParam(value = "memberTwo") String memberTwo,
                                             @RequestParam(value = "pageNo") int pageNo,
                                             @RequestParam(value = "pageSize") int pageSize
                                             ){
        ...
        Pageable paging = PageRequest.of(pageNo, pageSize, dateSort);
        List<MessageResponse> listOfMessages = new ArrayList<>();
